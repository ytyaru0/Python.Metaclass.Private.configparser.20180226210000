    ...
    def __SetAttributes(cls, target, name):
        ...
        for section in cls.config:
            for key in cls.config[section]:
                ...
                setattr(target, '_{}__{}{}'.format(name, s, k), cls.config[section][key]) # Db.Secretプロパティ定義
                setattr(target, '{}{}'.format(s, k), property(lambda target: getattr(target, '_{}__{}{}'.format(name, s, k))))
                print('_{}__{}{}'.format(name, s, k))
                print('{}{}'.format(s, k))
                print(hasattr(target, '_{}__{}{}'.format(name, s, k)), name, s, k)
                print(hasattr(target, '{}{}'.format(s, k)))
